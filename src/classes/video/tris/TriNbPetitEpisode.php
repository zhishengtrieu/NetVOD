...
class TriNbPetitEpisode implements Tri{

    // cette fonction permettera de trier les series du catalogue par nombre d'episode du plus petit au plus grand
    public function trier(): string{
        $res = "";
        //on créé un nouveau catalogue
        $catalogue = new Catalogue();
        //requete sql permettant de recuperer l'id et le titre et le nombre d'episode et on tri du plus petit au plus grand
        $query = "select serie.id,serie.titre, count(episode.id)from serie 
                inner join episode on serie.id = episode.serie_id 
                group by serie.id,titre order by count(episode.id) asc";
